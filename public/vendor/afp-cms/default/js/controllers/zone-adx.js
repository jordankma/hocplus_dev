...
angular
    .module("AdtechApp")
    .controller('zoneAdxCtrl', zoneAdxCtrl);

function zoneAdxCtrl($scope, $mdDialog, $http, $mdToast, removeByAttr) {
                ...
                $scope.addZoneAdx = function() {
                    $http({
                        ...
                        data: {
                            ...
                            bannerDefault: $scope.item.bannerDefaultLB,
                        }
                    }).success(function (response) {
                        console.log(response);
                        if(response.success==true) {
                ...
                controller: function ($scope, $mdDialog, $mdToast, items) {
                    ...
                    $scope.updateZoneAdx = function() {
                        $http({
                            ...
                            data: {
                                site_id: site_id,
                                zoneadx_id: $scope.item.zoneadx_id,
                                box_format_id: $scope.item.box_format_id,
                                zone_template_id: $scope.item.zone_template_id,
                                notes: $scope.item.notes,
                                status: $scope.item.statusLB,
        ...
        $mdDialog.show(confirm).then(function() {
            $http({
                ...
                if(response.success==true){
                    ...
                    $mdToast.show(
                        $mdToast.simple()
                            .textContent('Xoá thành công zone: '+name)
                            .theme("success-toast")
                            .position('top right')
                            .action('⛌')
                    );
