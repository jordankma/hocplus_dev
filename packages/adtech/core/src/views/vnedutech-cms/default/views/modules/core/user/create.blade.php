...
{{-- Page title --}}
@section('title'){{ $title = trans('adtech-core::titles.user.create') }}@stop
...
@section('header_styles')
    ...
    <link href="{{ config('site.url_static') . ('/vendor/' . $group_name . '/' . $skin . '/vendors/select2/css/select2.min.css') }}" type="text/css" rel="stylesheet">
    <link href="{{ config('site.url_static') . ('/vendor/' . $group_name . '/' . $skin . '/vendors/select2/css/select2-bootstrap.css') }}" rel="stylesheet">
    <link href="{{ config('site.url_static') . ('/vendor/' . $group_name . '/' . $skin . '/css/pages/wizard.css') }}" rel="stylesheet">
    <!--end of page css-->
@stop
            ...
            <div class="col-md-12">
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        <!--main content-->
                        <form id="commentForm" action="{{ route('adtech.core.user.add') }}"
                                                ...
                                                <select class="form-control required" title="Select group..." name="group"
                                                    ...
                                                    @foreach($groups as $group)
                                                        <option value="{{ $group->role_id }}"
                                                                @if($group->role_id == old('group')) selected="selected" @endif >{{ $group->name}}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-2">
                                                <div class="form-group">
                                                    <label>Status</label>
                                                    <div class="form-group">
                                                        {!! Form::checkbox('status', null, true, array('data-size'=> 'mini')) !!}
                                                    </div>
                                                </div>
                                            </div>
                                            {{--<div class="col-sm-2">--}}
...
@section('footer_scripts')
    ...
    <script src="{{ config('site.url_static') . ('/vendor/' . $group_name . '/' . $skin . '/vendors/bootstrapwizard/jquery.bootstrap.wizard.js') }}" type="text/javascript"></script>
    ...
    <script src="{{ config('site.url_static') . ('/vendor/' . $group_name . '/' . $skin . '/js/pages/add_user.js') }}"></script>
@stop
