...
@section('title'){{ $title = trans('afp-core::titles.tag.manage') }}@stop

@section('content')
    <div layout-no-padding="" ng-controller="tagCtrl">
        <md-card>
        ...
        <md-card style="padding-top: 10px">
            <md-table-container style="margin: 20px 0px">
                <table md-table multiple>
                    ...
                    <tr md-row>
                        ...
                        <th md-column md-text><span>{{ $labelName = trans('afp-core::common.tag.name') }}</span></th>
                        <th width="90" align="center" md-column
                            md-numeric>{{ $labelAction = trans('afp-core::common.action') }}</th>
                    </tr>
                    </thead>
                    <tbody md-body>
                    <tr md-row md-select="tag.id" md-select-id="tag.id" md-auto-select ng-repeat="tag in tagList">
                        <td md-cell>@{{tag.id}}</td>
                        <td md-cell>
                            <a ng-click="showEdit($event, tag.id)"><strong>@{{tag.name}}</strong></a>
                        </td>
                        <td md-cell>
                            <md-menu md-position-mode="target-right target">
                                ...
                                <md-menu-content width="4">
                                    <md-menu-item>
                                        <md-button ng-click="showEdit($event, tag.id)">
                                            <div layout="row" flex>
                                    ...
                                    <md-menu-item>
                                        <md-button ng-click="showDel($event, tag.id)">
                                            <div layout="row" flex>
            ...
            <md-table-pagination md-label="{page: '{{ trans('afp-core::labels.page') }}', rowsPerPage: '{{ trans('afp-core::labels.rowsPerPage') }}', of: '{{ trans('afp-core::labels.of') }}'}"
                                 ...
                                 md-total="@{{ query.total }}" md-on-paginate="getTags"
...
@push('scripts-view')
<script type="text/ng-template" id="frm-add-tag">
    <md-dialog flex="60">
        <md-toolbar>
            <div class="md-toolbar-tools">
                <h2>{{ trans('afp-core::titles.tag.add') }}</h2>
                <span flex></span>
        ...
        <md-dialog-content style="padding: 2em;">
            <form name="myForm">
                <md-input-container class="md-icon-float md-block">
                    ...
                    <input ng-model="tag.name" type="text" required>
                </md-input-container>
            </form>
        </md-dialog-content>
        <md-dialog-actions>
            <md-button ng-click="addTag()" ng-disabled="myForm.$invalid" class="md-raised md-primary">
                {{ trans('afp-core::buttons.save') }}
            </md-button>
        </md-dialog-actions>
    </md-dialog>
</script>
<script type="text/ng-template" id="frm-edit-tag">
    <md-dialog flex="60">
        <md-toolbar>
            <div class="md-toolbar-tools">
                <h2>{{ trans('afp-core::titles.tag.edit') }}</h2>
                <span flex></span>
                <md-button class="md-icon-button" ng-click="closeDialog()">
                    <i class="material-icons">close</i>
                </md-button>
        ...
        <md-dialog-content style="padding: 2em;">
            <form name="myForm">
                <md-input-container class="md-icon-float md-block">
                    <label>{{ trans('afp-core::labels.name') }}</label>
                    <input ng-model="tag.name" type="text" required>
                    <input ng-model="tag.id" type="hidden">
                </md-input-container>
            </form>
        </md-dialog-content>
        <md-dialog-actions>
            <md-button ng-click="showDel($event, tag.id)" class="md-raised md-warn">
                {{trans('afp-core::buttons.delete')}}
            </md-button>
            <md-button ng-click="updateTag()" ng-disabled="myForm.$invalid" class="md-raised md-primary">
                {{ trans('afp-core::buttons.save') }}
            </md-button>
        </md-dialog-actions>
    </md-dialog>
...
<script src="{{ url('/vendor/' . $group_name . '/' . $skin . '/js/controllers/tag.js?t=').time() }}"></script>
@endPush
