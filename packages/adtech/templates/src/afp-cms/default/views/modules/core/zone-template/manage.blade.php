...
@section('title'){{ $title = trans('afp-core::titles.zone-template.manage') }}@stop

@section('content')
    <div layout-no-padding="" ng-controller="zoneTemplateCtrl">
        <md-card>
                ...
                <table md-table multiple>
                    ...
                    <tr md-row>
                        ...
                        <th md-column md-text>
                            <span>{{ $labelName = trans('afp-core::common.zone-template.name') }}</span></th>
                    ...
                    <tr md-row md-select="item.id" md-select-id="item.id" md-auto-select ng-repeat="item in itemList">
                        ...
                        <td md-cell>
                            <a ng-click="showEdit($event, item.id)"><strong>@{{item.name}}</strong></a>
                        </td>
                        <td md-cell>
    ...
    <md-dialog flex="60">
        <md-toolbar>
            <div class="md-toolbar-tools">
                <h2>{{ trans('afp-core::titles.zone-template.add') }}</h2>
                <span flex></span>
        ...
        <md-dialog-content style="padding: 2em;">
            <form name="myForm">
                ...
                <md-input-container class="md-block">
                    <label>{{ trans('afp-core::labels.code') }}</label>
                    <input ng-model="item.code" type="text" required>
                </md-input-container>
            </form>
        </md-dialog-content>
        <md-dialog-actions>
            <md-button ng-click="addZone()" ng-disabled="myForm.$invalid" class="md-raised md-primary">
...
<script type="text/ng-template" id="frm-edit-item">
    <md-dialog flex="60">
        <md-toolbar>
            <div class="md-toolbar-tools">
                <h2>{{ trans('afp-core::titles.zone-template.edit') }}</h2>
                <span flex></span>
                <md-button class="md-icon-button" ng-click="closeDialog()">
                    <i class="material-icons">close</i>
                </md-button>
        ...
        <md-dialog-actions>
            ...
            <md-button ng-click="updateZone()" ng-disabled="myForm.$invalid" class="md-raised md-primary">
                {{ trans('afp-core::buttons.save') }}
            </md-button>
        </md-dialog-actions>
    </md-dialog>
...
<script type="text/javascript">
    var coreItem = $.parseJSON('{!! $jsonzoneTemplateString !!}');
    angular.module("AdtechApp")
        .run(function ($rootScope) {
            $rootScope.getBox = function () {
                var query = $rootScope.query;
                var url = '{{ route('afp.core.zone-template.list') }}',
...
<script src="{{ url('/vendor/' . $group_name . '/' . $skin . '/js/controllers/zone-template.js?t=').time() }}"></script>
@endPush
