...
$("#form-login").submit(function(event){
    ...
    $.post( url, { email: $('#login-email').val(), password: $('#login-password').val(), remember: $('#login-remember').val() }, function ( data ) {
        ...
        if (data.success) {
            if(data.status == 1){
                window.location.href = routeStudentProfile;
            } else if(data.status == 2){
                window.location.reload();
                // window.location.href = routeMyCourseStudent;
            }
        } else {
...
$("#form-forgot").submit(function(event){
    ...
    $.post( url, { email: $('#forgot-email').val() }, function ( data ) {
        ...
        if (data.success) {
            ...
            $(".form-restore-password").removeClass('show');
            $(".form-otp").addClass('show');
            // setTimeout(function () {
            //     window.location.href = "/";
            // }, 3000);
        } else {
...
$("#form-otp-password").submit(function(event){
    event.preventDefault();

    /* get the action attribute from the <form action=""> element */
    var routeIndex = routeIndex;
    var $form = $( this ),
    url = $form.attr( 'action' );

    /* Send the data using post with element id name and name2*/
    $.post( url, { otp: $('#otp').val() }, function ( data ) {
        data = jQuery.parseJSON( data );
        if (data.success) {

        	let token = data.token;
            setTimeout(function () {
            	// window.location.replace("reset-password/" + token);
                window.location.href = "/reset-password/" + token;
            }, 3000);
        } else {
            $('#forgot-btn-submit').attr('disabled', false);
            $("#otp-notification-err").css('display', 'block');
        }
        // return false;
    });
});
