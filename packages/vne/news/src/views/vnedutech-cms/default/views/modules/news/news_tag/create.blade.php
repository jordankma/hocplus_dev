...
@section('content')
    ...
    <section class="content paddingleft_right15">
            <div class="the-box no-border">
                <div class="row">
                        <form action="{{route('vne.news.tag.add')}}" method="post" id="form-add-cat">
                            <div class="col-md-5" style="">
                                <div class="form-group ui-draggable-handle" style="position: static;">
                                    <label for="input-text-1">{{trans('vne-news::language.label.name')}}</label>
                                    <input type="text" name="name" class="form-control" id="input-text-1" placeholder="{{trans('vne-news::language.form.tags_placeholder')}}">
                                </div>
                            </div>
                            <div class="form-group col-md-12">
                                <button type="submit" class="btn btn-success">{{trans('vne-news::language.buttons.create')}}</button>
                                <a href="{{route('vne.news.tag.manager')}}" class="btn btn-danger">{{trans('vne-news::language.buttons.discard')}}</a>
                            </div>
                        </form>
                </div>
            </div>
