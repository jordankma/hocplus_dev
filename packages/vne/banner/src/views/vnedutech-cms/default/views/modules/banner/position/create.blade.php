...
{{-- Page title --}}
@section('title'){{ $title = trans('vne-banner::language.titles.position.create') }}@stop
    ...
    <section class="content paddingleft_right15">
        ...
        <div class="row">
            <div class="the-box no-border">
                <!-- errors -->
                <form role="form" action="{{route('vne.banner.position.add')}}" method="post" id="form-add-position">
                <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                <div class="row">
                    <div class="col-sm-4">
                        <label>{{trans('vne-banner::language.label.name') }} <span style="color: red">(*)</span></label>
                        <div class="form-group">
                            <input type="text" name="name" class="form-control" placeholder="{{trans('vne-banner::language.placeholder.position.name') }}">
                        </div>
                        <label>{{trans('vne-banner::language.label.width') }} x {{trans('vne-banner::language.label.height') }} <span style="color: red">(*)</span></label>
                        <div class="form-group row">
                            <div class="col-md-5">
                                <input type="nember" min="0" name="width" class="form-control" placeholder="{{trans('vne-banner::language.placeholder.position.width') }}">
                            </div>
                            <div class="col-md-1">
                                x
                            </div>
                            <div class="col-md-5">
                                <input type="number" min="0" name="height" class="form-control" placeholder="{{trans('vne-banner::language.placeholder.position.height') }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="form-group">
