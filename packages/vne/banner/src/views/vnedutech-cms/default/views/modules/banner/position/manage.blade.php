...
{{-- Page title --}}
@section('title'){{ $title = trans('vne-banner::language.titles.position.manage') }}@stop
            ...
            <div class="panel panel-primary ">
                <div class="panel-heading clearfix">
                    ...
                    <div class="pull-right">
                        @if ($USER_LOGGED->canAccess('vne.banner.position.create'))
                        <a href="{{ route('vne.banner.position.create') }}" class="btn btn-sm btn-default"><span
                                    class="glyphicon glyphicon-plus"></span> {{ trans('vne-banner::language.buttons.create') }}</a>
                        @endif
                    </div>
                </div>
                <br/>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="table">
                            ...
                            <tr class="filters">
                                ...
                                <th>{{ trans('vne-banner::language.table.banner.name') }}</th>
                                <th class="fit-content" style="width: 120px">{{ trans('vne-banner::language.table.action') }}</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
    ...
    <script>
        $(function () {
            var table = $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('vne.banner.position.data') }}',
                columns: [
